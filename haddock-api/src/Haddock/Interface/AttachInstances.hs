...
attachInstances :: InterfaceBase -> PackageBase -> Ghc ()
attachInstances ifaces packages = do
  undefined ifaces packages
  ...
  -- index <- do
  --   hsc_env <- getSession
  --   mods_visible <- liftIO $ getModsVisible ifaces
  --   (_msgs, mb_index) <- liftIO $ runTcInteractive hsc_env $ do
  ...
  --   pure $ fromMaybe empty_index mb_index

  -- liftIO $ forInterfaces ifaces $ runGhc . void . attach index
  -- where
  ...
  --     -- let getInstDoc = findInstDoc iface ifaces packages
  --     --     getFixity = findFixity iface ifaces packages

  --     newItems <- mapM (attachToExportItem cls_insts fam_insts inst_map ifaces)
  --                      (ifaceExportItems iface)
  --     let orphanInstances = attachOrphanInstances ifaces (ifaceInstances iface) fam_insts
